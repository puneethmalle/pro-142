...
def popular_articles():
    ...
    for index,row in output.iterrows():
        ...
        pop_articles.append(p)
    return jsonify({
        "data": pop_articles,
        "status": "success"
    }) 

# API to return top 10 similar articles sing content based filtering method.
@app.route("/recommended-articles")
def recommended_articles():
    global liked_articles
    col_names = ['url','title','text','lang','total_events']
    recommended_article = pd.DataFrame(columns = col_names)
    for like_articles in liked_articles:
        output = get_recommendations(like_articles["title"])
        recommended_article = recommended_article.append(output)
    recommended_article.drop_duplicates(subset=["title"],inplace=True)
    recommended_article_data = []
    for index,row in recommended_article.iterrows():
        p = {
            "url":row["url"],
            "title":row["title"],
            "text":row["text"] or "N/A",
